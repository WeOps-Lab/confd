...
func init() {
	...
	flag.StringVar(&config.HTTPBackendFormat, "http-backend-format", "json", "HTTP backend format")
	flag.StringVar(&config.HTTPBackendUrl, "http-backend-url", "", "HTTP backend URL")
	flag.IntVar(&config.HttpBackendScrapInterval, "http-backend-scrap-interval", 0, "HTTP backend scrap interval")
